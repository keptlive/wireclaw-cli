...
<dict>
    ...
    <string>${projectRoot}/logs/wireclaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/wireclaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.wireclaw');
  } catch {
...
/**
 * Kill any orphaned wireclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned wireclaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/wireclaw.service';
    ...
    unitPath = path.join(unitDir, 'wireclaw.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=WireClaw Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/wireclaw.log
StandardError=append:${projectRoot}/logs/wireclaw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned wireclaw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable wireclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start wireclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active wireclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-wireclaw.sh');
  const pidFile = path.join(projectRoot, 'wireclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-wireclaw.sh — Start WireClaw without systemd',
    ...
    '    echo "Stopping existing WireClaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting WireClaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/wireclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/wireclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "WireClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/wireclaw.log"`,
  ];
