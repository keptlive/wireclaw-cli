...
Findings from reverse-engineering `@anthropic-ai/claude-agent-sdk` v0.2.29–0.2.34 to understand how `query()` works, why agent teams subagents were being killed, and how to fix it. Supplemented with official reference docs.
...
| `pathToClaudeCodeExecutable` | `string` | Uses built-in | Path to the CLI executable |
...
| `systemPrompt` | `string \| { type: 'preset'; preset: 'claude_code'; append?: string }` | `undefined` | System prompt. Use preset to get the default prompt, with optional `append` |
