...
│  │    • mcp__wireclaw__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
```
wireclaw/
├── CLAUDE.md                      # Project context for the CLI
├── docs/
...
│
├── container/
│   ├── Dockerfile                 # Container image (runs as 'node' user, includes the CLI)
...
│   ├── wireclaw.log               # Host stdout
│   └── wireclaw.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.wireclaw.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'wireclaw-agent:latest';
...
```
The token can be extracted from `~/.claude/.credentials.json` if you're logged in to the CLI.
...
- `{{PROJECT_ROOT}}` - Absolute path to your wireclaw installation
...
WireClaw uses a hierarchical memory system based on CLAUDE.md files.
...
1. **Agent Context Loading**
   ...
   - Claude SDK with `settingSources: ['project']` automatically loads:
...
2. Session ID is passed to Claude SDK's `resume` option
...
6. Router catches up conversation:
   ...
   │
   ▼
7. Router invokes Claude SDK:
   ├── cwd: groups/{group-name}/
   ├── prompt: conversation history + current message
   ├── resume: session_id (for continuity)
   └── mcpServers: wireclaw (scheduler)
   │
   ▼
...
WireClaw has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__wireclaw__schedule_task]
        {
...
### WireClaw MCP (built-in)

The `wireclaw` MCP server is created dynamically per agent call with the current group's context.
...
WireClaw runs as a single macOS launchd service.

### Startup Sequence

When WireClaw starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned WireClaw containers from previous runs
...
### Service: com.wireclaw

**launchd/com.wireclaw.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/wireclaw.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/wireclaw.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.wireclaw.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.wireclaw.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.wireclaw.plist

# Check status
launchctl list | grep wireclaw

# View logs
tail -f logs/wireclaw.log
```
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep wireclaw` |
| "Agent process exited with code 1" | Container runtime failed to start | Check logs; WireClaw auto-starts container runtime but may fail |
...
- `logs/wireclaw.log` - stdout
- `logs/wireclaw.error.log` - stderr
