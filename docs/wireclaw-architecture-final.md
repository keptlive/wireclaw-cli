...
Before any operation, the system copies all files that will be modified to `.wireclaw/backup/`. On success, the backup is deleted. On failure, the backup is restored. This provides rollback safety regardless of whether the user commits, pushes, or understands git.

---

## 1. The Shared Base

`.wireclaw/base/` holds the clean core — the original codebase before any skills or customizations were applied. This is the stable common ancestor for all three-way merges, and it only changes on core updates.
...
- Auditable: `diff .wireclaw/base/src/server.ts skills/add-whatsapp/modify/src/server.ts` shows exactly what the skill changes
- Deterministic: same three inputs always produce the same merge result
- Size is negligible since WireClaw's core files are small
...
applied_skills:
  - skill: telegram
    version: 1.0.0
    custom_patch: .wireclaw/custom/telegram-group-only.patch
...
Copy all files that will be modified to `.wireclaw/backup/`. If the operation fails at any point, restore from backup.
...
```bash
git merge-file src/server.ts .wireclaw/base/src/server.ts skills/add-whatsapp/modify/src/server.ts
```
...
If tests pass, delete `.wireclaw/backup/`. The operation is complete.

If tests fail and Level 2 can't resolve, restore from `.wireclaw/backup/` and report the failure.
...
WireClaw maintains a verified resolution cache in `.wireclaw/resolutions/` that ships with the project. This is the shared artifact — **not** `.git/rr-cache/`, which stays local.
...
git merge-file current.ts .wireclaw/base/src/file.ts skills/add-whatsapp/modify/src/file.ts

# 2. If exit code > 0 (conflict), set up rerere adapter:

# Create blob objects for the three versions
base_hash=$(git hash-object -w .wireclaw/base/src/file.ts)
...
`.wireclaw/state.yaml` records everything about the installation:
...
custom_modifications:
  - description: "Added custom logging middleware"
    ...
    files_modified:
      - src/server.ts
    patch_file: .wireclaw/custom/001-logging-middleware.patch
```
...
Core updates must be as programmatic as possible. The WireClaw team is responsible for ensuring updates apply cleanly to common skill combinations.
...
Copy all files that will be modified to `.wireclaw/backup/`.
...
```bash
git merge-file src/server.ts .wireclaw/base/src/server.ts updates/0.5.0-to-0.6.0/files/src/server.ts
```
...
```bash
git apply --3way .wireclaw/custom/001-logging-middleware.patch
```
...
`.wireclaw/base/` replaced with new clean core. This is the **only time** the base changes.
...
3. Backup the current codebase to `.wireclaw/backup/`
...
2. Updates `.wireclaw/base/` to the current core version's clean files
...
load-resolutions .wireclaw/resolutions/
...
for skill in state.applied_skills:
  ...
  for file in skill.files_modified:
    ...
    git merge-file ${resolved_path} .wireclaw/base/${resolved_path} skills/${skill.name}/modify/${file}
...
Ship with WireClaw to reduce noisy merge conflicts:
...
5. **One shared base.** `.wireclaw/base/` is the clean core before any skills or customizations. It's the stable common ancestor for all three-way merges. Only updated on core updates.
...
7. **Resolutions are learned and shared.** Maintainers resolve conflicts and ship verified resolutions with hash enforcement. `.wireclaw/resolutions/` is the shared artifact.
