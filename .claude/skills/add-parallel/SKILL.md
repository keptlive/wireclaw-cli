...
Adds Parallel AI MCP integration to WireClaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses WireClaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. WireClaw already set up and running
...
Add Parallel AI MCP servers after the wireclaw server:
```typescript
const mcpServers: Record<string, any> = {
  wireclaw: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__wireclaw__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__wireclaw__send_message
   4. Use mcp__wireclaw__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo wireclaw-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep wireclaw  # macOS
...
- Check task runs: `tail -f logs/wireclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.wireclaw` (macOS) or `systemctl --user restart wireclaw` (Linux)
