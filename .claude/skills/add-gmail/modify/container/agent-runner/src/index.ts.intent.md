...
- Added: `gmail` MCP server alongside the existing `wireclaw` server:
  ```
  gmail: {
    command: 'npx',
...
## Invariants
- The `wireclaw` MCP server configuration is unchanged
...
## Must-keep
- The `wireclaw` MCP server with its environment variables
