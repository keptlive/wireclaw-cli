...
This skill adds Telegram support to WireClaw using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.wireclaw/state.yaml`. If `telegram` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
4. Service is running: `launchctl list | grep wireclaw` (macOS) or `systemctl --user status wireclaw` (Linux)
...
- Check bot is started: `tail -f logs/wireclaw.log`
...
# Linux:
# systemctl --user stop wireclaw
# npm run dev
# systemctl --user start wireclaw
```
...
6. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.wireclaw` (macOS) or `npm run build && systemctl --user restart wireclaw` (Linux)
