---
name: x-integration
description: X (Twitter) integration for WireClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** WireClaw v1.0.0. Directory structure may change in future versions.
...
1. **WireClaw is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep wireclaw (macOS) or systemctl --user status wireclaw (Linux)
```
...
| `WIRECLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/wireclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into WireClaw, make the following modifications:
...
All paths below are relative to project root (`WIRECLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep wireclaw  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status wireclaw
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/wireclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/wireclaw.log | tail -20
```
...
docker run wireclaw-agent ls -la /app/src/skills/
```
