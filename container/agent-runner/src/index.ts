...
import { getAdapter } from './frameworks/registry.js';
...
// Framework-specific query logic is in ./frameworks/ adapters.
// The adapter is selected based on containerInput.framework.
...
async function main(): Promise<void> {
  ...
  // Instantiate the framework adapter based on manifest config
  const framework = (containerInput as any).framework || 'claude-code';
  const adapter = getAdapter(framework, { log, writeOutput, shouldClose });
  log(`Using framework adapter: ${adapter.name}`);
  ...
  try {
    while (true) {
      ...
      const queryResult = await adapter.runQuery({ prompt, sessionId, mcpServerPath, containerInput, sdkEnv, resumeAt });
